...
export default function Notice() {
    ...
    const getboard = async () => {
        const {data} = await axios.post('http://localhost/list/bbs',{category:'공지사항',page:page})
        console.log(data);
        setList(data.list);
    ...
    const getMyList = async() => {
        const {data} = await axios.post('http://localhost/list/bbs',{category:'공지사항',page:page,user_id:sessionStorage.getItem('user_id')})
        console.log(data);
        setMyList(data.list);
    ...
    const MoveBoardWrite = () => {
        router.push('/component/board/boardwrite?category=공지사항');
    }

    const MoveBoardDetail = (board_idx) =>{
        router.push('/component/board/boarddetail?category=공지사항&board_idx='+board_idx);
    }
        ...
        <div>
            ...
            <div className='wrap padding_120_0'>
                <div className='flex justify_con_between'>
                    ...
                    {user_level > 3 ? <div className='btn label white_color' style={{width:'auto'}} onClick={MoveBoardWrite}>글쓰기</div> : ''}
                </div>
                <div className='mt_20'>
