...
export default function QnA() {
    ...
    const getboard = async () => {
        const {data} = await axios.post('http://localhost/list/bbs',{category:'QnA',page:page})
        console.log(data);
        setList(data.list);
    ...
    const getMyList = async() => {
        const {data} = await axios.post('http://localhost/list/bbs',{category:'QnA',page:page,user_id:sessionStorage.getItem('user_id')})
        console.log(data);
        setMyList(data.list);
    ...
    const MoveBoardWrite = () => {
        router.push('/component/board/boardwrite?category=QnA');
    }

    const MoveBoardDetail = (board_idx) =>{
        router.push('/component/board/boarddetail?category=QnA&board_idx='+board_idx);
    }
                ...
                <div className='mt_20'>
                    ...
                    {filteredList && page <= filteredTotal && (
                        filteredList.map((l, idx) => (
                            <div key={l.board_idx} className='flex justify_con_space_between gap_10 board_css board_content' onClick={()=>MoveBoardDetail(l.board_idx)}>
